...
export default function HoneyPage() {
  const storedHoney = JSON.parse(localStorage.getItem("honey")) || [];
  const [products, setProducts] = useState([
    ...initialProducts.filter((p) => p.category === "honey"),
    ...storedHoney,
  ]);
  ...
  const handleAddProduct = (e) => {
    ...
    reader.onload = () => {
      ...
      const existing = JSON.parse(localStorage.getItem("honey")) || [];
      localStorage.setItem("honey", JSON.stringify([...existing, newProduct]));
  ...
  const handleDelete = (id) => {
    ...
    localStorage.setItem(
      "honey",
      JSON.stringify(updated.filter((p) => p.category === "honey"))
    );
  };

  return (
    <div className="p-6 min-h-screen bg-gray-100">
      <h2 className="text-3xl font-bold mb-5 capitalize">Organic Honey</h2>

      {farmerUser && (
        <div className="mb-6 p-4 bg-white rounded-xl shadow">
          <h3 className="font-semibold mb-3">Add Your Honey Product</h3>
          <form
          ...
          {preview && (
            <img
              src={preview}
              alt="Preview"
              className="mt-4 object-cover rounded w-[250px] h-[250px]" 
            />
          )}
        </div>
      )}
